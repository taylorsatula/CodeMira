...
from codemira.extraction.chunker import get_token_count, build_extraction_chunks, parse_turns
...
class TestEstimateTokenCount:
    ...
    def test_basic(self):
        text = "a" * 400
        assert get_token_count(text) == 100
...
class TestSplitIntoTurns:
    def test_single_user_message(self):
        assert parse_turns("User: hello") == ["User: hello"]

    def test_user_assistant_pair(self):
        transcript = "User: hello\nAssistant: hi"
        assert parse_turns(transcript) == ["User: hello\nAssistant: hi"]
...
class TestChunkCompressedTranscript:
    def test_short_transcript_returns_single_chunk(self):
        transcript = "User: hello\n\nAssistant: hi"
        chunks = build_extraction_chunks(transcript, context_length=128000)
        assert chunks == [transcript]

    def test_empty_transcript(self):
        chunks = build_extraction_chunks("", context_length=128000)
        assert chunks == [""]

    def test_splits_at_user_message_boundaries(self):
        turn = "User: " + "x" * 8000 + "\nAssistant: reply"
        transcript = "\n\n".join([turn] * 50)
        chunks = build_extraction_chunks(transcript, context_length=32000)
        assert len(chunks) > 1
        for chunk in chunks:
    ...
    def test_chunk_size_respects_context_length(self):
        ...
        chunks = build_extraction_chunks(transcript, context_length)
        for chunk in chunks:
            estimated_tokens = get_token_count(chunk)
    ...
    def test_seventy_percent_preferred_over_75k_for_large_context(self):
        ...
        for chunk in chunks:
            assert get_token_count(chunk) <= expected_budget
    ...
    def test_seventy_five_k_floor_for_small_context(self):
        ...
        chunks = build_extraction_chunks(transcript, context_length)
        for chunk in chunks:
            assert get_token_count(chunk) <= budget

    def test_existing_memories_reduces_budget(self):
        turn = "User: " + "x" * 4000 + "\nAssistant: reply"
        transcript = "\n\n".join([turn] * 100)
        context_length = 32000
        chunks_no_existing = build_extraction_chunks(transcript, context_length, existing_memories_token_estimate=0)
        chunks_with_existing = build_extraction_chunks(transcript, context_length, existing_memories_token_estimate=2000)
    ...
    def test_single_turn_exceeding_budget_stays_intact(self):
        ...
        chunks = build_extraction_chunks(huge_turn, context_length=128000)
        assert len(chunks) == 1
