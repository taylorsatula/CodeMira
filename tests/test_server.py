...
def server_setup(tmpdir_path):
    ...
    project_root = os.path.join(tmpdir_path, "proj")
    ...
    server_thread.start()
    time.sleep(0.1)
    yield f"http://localhost:{port}", conn, mi, ids, embs, config, project_root
    server.shutdown()
    manager.close_all()
...
class TestRetrieveEndpoint:
    def test_retrieve_returns_memories(self, server_setup):
        base_url, conn, mi, ids, embs, config, project_root = server_setup
        payload = json.dumps({
...
class TestArcGetEndpoint:
    def test_arc_returns_null_when_no_arc(self, server_setup):
        base_url, conn, mi, ids, embs, config, project_root = server_setup
        resp = urllib.request.urlopen(f"{base_url}/arc?session_id=ses_none&project_root={urllib.parse.quote(project_root)}")
        data = json.loads(resp.read())
        assert data["arc"] is None
        assert data["session_id"] == "ses_none"

    def test_arc_returns_stored_arc(self, server_setup):
        base_url, conn, mi, ids, embs, config, project_root = server_setup
        ...
        resp = urllib.request.urlopen(f"{base_url}/arc?session_id=ses_test&project_root={urllib.parse.quote(project_root)}")
        data = json.loads(resp.read())
        assert data["arc"] == "[START] Goal: Fix bug\n └─ [CURRENT] Done"
