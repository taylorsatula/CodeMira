...
**Where**: `tests/test_summarization.py:61, 66` invoke `_chunk_transcript(transcript, 128000)` with 2 positional args. The function signature in `summarization/handler.py:39` is `_chunk_transcript(transcript: str, context_length: int, chunk_target_tokens: int)` — `chunk_target_tokens` has no default.
...
**Real fix**: Either (a) add a default value `chunk_target_tokens: int = 30_000` to match how `generate_arc_summary` calls it (`handler.py:73-74`), or (b) update the test calls to pass the third arg explicitly. Bundle 5 is rewriting the chunking layer anyway and may collapse this — revisit then.
