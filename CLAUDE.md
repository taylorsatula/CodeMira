...
- **Credential Management**: `CODEMIRA_EXTRACTION_API_KEY` is the only required external secret (the API key for the extraction LLM endpoint). It must be baked into the launchd plist's `EnvironmentVariables` at install time — launchd agents do not inherit the user's shell env. `daemon/codemira/launchd.py` fails at install if the key isn't exported. If the key is missing at runtime, extraction logs an error and the cycle skips; the daemon does not substitute a fallback. Per-role base URLs and optional API keys live in the same `CODEMIRA_*` namespace (`*_BASE_URL`, `*_API_KEY` for `subcortical`/`consolidation`/`arc`/`extraction`).
- **Fail-Fast Infrastructure**: Required infrastructure failures MUST propagate. Do not catch exceptions from SQLite, hnswlib, embeddings, the HTTP bridge, or remote LLM endpoints and return `None`/`[]`/defaults — that masks outages as normal operation. `try/except` is only for: (1) adding context before re-raising, (2) legitimately optional paths (link classification falls back to `corroborates` if the subcortical LLM is down; consolidation skips a cluster on model error), (3) graceful degradation at the process boundary (plugin sets `daemonUnavailable` and stops injecting HUDs). An empty list means "no memories found". A raised exception means "infrastructure broke". Keep the distinction.
...
- *Pure / no-I/O*: `get_*`, `is_*`, `has_*`, `format_*`, `render_*`, `parse_*`, `build_*`, `pick_*`, `collect_*`, `iter_*`, `dedupe_*`.
- *Local I/O (single SQLite read/write, file open/close, in-process index)*: `read_*`, `insert_*`, `update_*`, `upsert_*`, `delete_*`, `link_*`, `archive_*`, `log_*`, `search_*`, `open_*`, `close_*`, `init_*`.
...
`is_*` / `has_*` are pure by default but may execute a single bounded local read when the function exists *only* to answer a yes/no question (`is_session_extracted` running one `SELECT 1`). If the predicate fans out to multiple queries or any network call, rename it.

Banned function-name verbs: `process`, `handle`, `do`, `note`, `apply`, `execute`, `find`, `discover`. They predict nothing about cost or behavior — collapse to a cost-class verb (`read_*` for "find from DB", `pick_*` for "choose among candidates", `collect_*` for "gather across reads", `build_*` for "compute from inputs"). `run_*` is reserved for top-level entry points (`run_daemon`, `run_consolidation`) — never mid-pipeline.

**Standardized verb pairings (avoid synonyms):**
- DB read → `read_*`, never `get_*` (which is pure-only). `get_*` is for in-memory accessors and pure derivations only.
- Serialize/deserialize → `format_*` / `parse_*`, never `to_*` / `from_*` (`format_embedding_blob`, not `embedding_to_blob`).
- Insert-or-update → `upsert_*`, never `get_or_create_*` or ad-hoc compounds.
- Background dispatch → `start_*` (returns immediately after spawning) or `spawn_*` (raw thread/process). Never `run_*`, never `handle_*`.
- HTTP route handlers → name by action, not by HTTP verb or "handle" prefix (`_collect_health_status`, `_read_arc`, `_start_extraction`).
...
### Fail-Fast Bootstrap
- Missing `CODEMIRA_EXTRACTION_API_KEY`: `launchd install` refuses to write the plist.
- Missing prompt files: `FileNotFoundError` at the call site (prompts loaded lazily per call, not at startup).
- Missing LLM endpoint (e.g. local Ollama is down): daemon logs + skips the compression/extraction/link/consolidation step that needed it; plugin sets `daemonUnavailable` and stops injecting HUDs.
...
- Malformed `*_base_url` (no `/v1` path component): `call_llm` raises `ValueError` at the call site; daemon does not issue a poisonous request.
...
- **On-demand generation**: The plugin fires `POST /arc/generate` (fire-and-forget, no await) when it detects a new user message. The daemon spawns a background thread that reads the full conversation from OpenCode's SQLite, formats it into a lightweight flat transcript (no LLM compression pass), chunks it if it exceeds the model context window, and runs `call_llm()` per chunk with frozen-prefix incremental chunking. The result is stored in the `arc_fragments` table in the project's `memories.db`.
...
- **No double-compression**: The arc summarizer formats the raw conversation directly (tool name + title/output[:500]) instead of running tool compression via `build_tool_compressor`. The arc model produces its own structural abstraction — pre-compressing adds LLM round-trips with no benefit.
...
### LLM Transport via stdlib HTTP
All LLM calls — subcortical, extraction, consolidation, arc — go through a single `call_llm()` function in `daemon/codemira/llm.py` using `urllib.request`. The function POSTs to `{base_url}/chat/completions` with the standard OpenAI Chat Completions request shape (`{model, messages, stream:false}`); `Authorization: Bearer {api_key}` is added only if `api_key` is non-empty. Any provider speaking the OpenAI Chat Completions spec works (Ollama via its `/v1` endpoint, OpenRouter, OpenAI, vLLM, llama.cpp's HTTP server, LM Studio, etc.). No SDK dependency. The plugin side uses `fetch()` against the same OpenAI-compatible shape for the same reason (stdlib everywhere). Each role has a `(model, base_url, api_key)` config triplet — there is no provider flag, no auto-detection. `base_url` MUST include the API path (e.g. `http://localhost:11434/v1`); `call_llm` raises `ValueError` if the path is missing.

### Chunked Extraction for Long Conversations
`build_extraction_chunks()` in `daemon/codemira/extraction/chunker.py` splits the compressed transcript when it exceeds the extraction model's context window. Chunk target = `max(75_000, 0.7 * extraction_model_context_length)` tokens, minus prompt overhead (`PROMPT_OVERHEAD_TOKENS = 2048`) and existing-memories token estimate. Chunks are split at user-message boundaries (`User:` prefixes) via `parse_turns()` — each turn includes its user message and all subsequent `Assistant:`/`Tool:` lines until the next `User:` line, so tool call chains are never split mid-stream. Each chunk is extracted sequentially via `extract_memories()` with `prior_chunk_texts` — memories from earlier chunks are injected into the `{existing_memories}` prompt section under a `--- Previously extracted from this session ---` separator, and dedup checks run against both DB memories and prior-chunk texts. This prevents duplication while enriching context for later chunks. Short conversations that fit in one chunk are unchanged (single-element list returned).

### Entity Extraction is LLM-Based
`daemon/codemira/extraction/dedup.py:extract_entities` calls the configured `subcortical_model` via `call_llm` with `prompts/entity_extraction_*.txt`. The model returns a JSON array of `{name, type}` entries. `name` is lowercased and deduped; `type` is validated against `VALID_ENTITY_TYPES` and falls back to `"other"`. `VALID_ENTITY_TYPES` = `{library, framework, tool, pattern, protocol, error, project_concept, other}`. Project-specific named concepts (modules, subsystems, project-internal terms like "peanutgallery", "domaindocs") are `project_concept` — the highest-value entity type for hub discovery because they link to `vocabulary` category memories that ecosystem terms cannot surface. Callers pass `model`, `base_url`, `api_key`, and `prompts_dir` explicitly — there are no defaults.

### Hybrid Retrieval
`HybridSearcher.hybrid_search()` issues BM25 (via FTS5) + ANN (via hnswlib) with `limit * 2` each, merges via Reciprocal Rank Fusion (`RRF_K=60`), and caps at `limit`. Fresh search results are augmented by `collect_hub_memories()` (entity-indexed + link-graph memories) and pinned memories from the previous iteration. Final cap: `max_surfaced_memories` (default 8).

### Extraction Error Taxonomy
`ExtractionError` (`daemon/codemira/errors.py`) marks non-retryable extraction failures — the LLM returned unparseable output or an unexpected response structure. Retrying the same conversation will likely produce the same result, so the session is logged to `extraction_log` and skipped on future cycles. Infrastructure failures (`URLError`, `HTTPError`, `sqlite3.OperationalError`) are NOT `ExtractionError` — they propagate to allow retry when the outage resolves. `call_llm()` raises `ExtractionError` for bad JSON bodies or missing keys in otherwise successful HTTP responses; network errors still propagate as `URLError`/`HTTPError`.

### Extraction Skip Mechanism
The `extraction_log` table (`session_id`, `extracted_at`, `memory_count`, `attempt_count`, `is_complete`) prevents re-processing. `is_complete = 1` means the session is done (successfully extracted, non-retryable error, or max attempts exhausted). `is_complete = 0` with `attempt_count < max_extraction_attempts` means the session had an infrastructure failure and should be retried. `_collect_extracted_session_ids` only returns `is_complete = 1` rows. Every code path out of `extract_session_memories` MUST call `log_extraction()` — complete paths with `is_complete=True`, infra-failure paths with `is_complete=False`. After `is_complete=False` logging, if `attempt_count >= max_extraction_attempts` (default 3), `update_extraction_complete()` marks the session unextractable and the exception is suppressed (no re-raise). Otherwise the exception propagates for retry on the next poll cycle.
...
- **Install launchd agent**: `python -m codemira.launchd install` (requires `CODEMIRA_EXTRACTION_API_KEY` exported)
...
**Right**: Every exit path calls `log_extraction(memory_conn, session_id, count)`. For `ExtractionError` catch with count 0. For infra-failure paths with `is_complete=False`; then check `attempt_count >= max_extraction_attempts` and call `update_extraction_complete()` if so, otherwise re-raise.
...
**Right**: Collecting `CODEMIRA_EXTRACTION_API_KEY` (and any other `CODEMIRA_*` / `OPENCODE_DB`) at install time and XML-escaping them into the plist's `EnvironmentVariables` dict. The required-key gate runs first; the rest are collected by sweeping `os.environ` for the `CODEMIRA_` prefix.
