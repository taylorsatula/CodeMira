...
- **No double-compression**: The arc summarizer formats the raw conversation directly (tool name + title/output[:500]) instead of running tool compression via `tool_compressor`. The arc model produces its own structural abstraction — pre-compressing adds LLM round-trips with no benefit.
